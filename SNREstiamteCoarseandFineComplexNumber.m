...
clear;
clc;
IQ_val = read_file('SNREstimate90dBexternalAttn_31pt5Attnsnth_900pt5Freq.bin');


% IQfft = fft(IQ_val);
% IQffttrunc = IQfft(1e6:4e6);
% IQ_val = ifft(IQffttrunc);
...
IQ_real_norm = IQ_real/max(abs(IQ_real));
IQ_real_norm =IQ_real_norm';
% IQ_imag_norm = IQ_imag/max(abs(IQ_imag));
...
for a_val = amp_start:amp_step:amp_end
   ...
   for freq_val=freq_start:freq_step:freq_end
      ...
      for phase_val = phase_start:phase_step:phase_end
         k=k+1;
         %a_val
         signal_estimate = a_val*exp(1i*2*pi*freq_val*ts*[1:N] + phase_val);
         real_error_val(i,j,k) = mean(abs((IQ_real_norm - signal_estimate)).^2);
...
for a_val = amp_start:amp_step:amp_end
   ...
   for freq_val=freq_start:freq_step:freq_end
      ...
      for phase_val = phase_start:phase_step:phase_end
         ...
         %a_val
         signal_estimate = a_val*exp(1i*2*pi*freq_val*ts*[1:N] + phase_val);
         real_error_val_fine(i,j,k) = mean(abs((IQ_real_norm - signal_estimate)).^2);
...
real_fine_signal_estimate = true_amp_real_fine*exp(1i*2*pi*true_freq_real_fine*ts*[1:N] + true_phase_real_fine);
%imag_best_signal_estimate = amp_range(i_imag)*fs(2*pi*freq_range(j_imag)*ts*[1:N] + phase_range(k_imag));


% error_signal_real = IQ_real_norm - real_best_signal_estimate;
SNR_estimate_real = 10*log10(mean(abs((real_fine_signal_estimate)).^2)/mxv_real)
